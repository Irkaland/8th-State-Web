...
import { LAB_REG_COPY, LAB_REG_FIELDS } from "@/content/lab-registration";
...
export default async function StudioLabPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <DaoShell locale={locale} messages={m} veil="green" footer footerGround="dark">
      {/* the one client boundary on this route: it owns the registration
          file and hands it to every REGISTER control below. The page itself
          stays a server component and renders fully without JavaScript. */}
            ...
            <div className="dsl__herobody">
              <div className="dsl__lede">
                ...
                <div className="dsl__heroctas">
                  <LabRegisterButton className="dsl__cta">
          ...
          <section className="dsl__program" id="program" aria-labelledby="dsl-s04">
            ...
            {LAB_PROGRAM.map((p) => (
              <div key={p.slug} className="dsl__prowrap">
                {/* §J: the row is a CONTAINER, not a link. It carries two
                    separate destinations - the course sheet and the
                    registration file - and the approved design nests the
                    second inside the first, which puts a control inside an
                    anchor: unreachable by keyboard, and announced as one link
                    with both labels run together. The course name is the link
                    and REGISTER is its own button beside it; the row keeps the
                    same grid, and its hover shift now belongs to the row. */}
                <div className="dsl__prow">
                  <span className="dsl__prno" aria-hidden="true">
                    {p.n}
                  </span>
                  <span className="dsl__prname">
                    <Link href={href(p.slug)} className="dsl__prtitle">
                      {courseName(p.slug)}
                    </Link>
                  ...
                  <LabRegisterButton
                    course={p.slug as LabCourseId}
                    className="dsl__prreg"
                    label={`${t(LAB_REG_COPY.registerFor, locale)} — ${courseName(p.slug)}`}
                  >
                    {up(R.register)}
                    <LabArrow weight={1.4} />
                  </LabRegisterButton>
          ...
          <section className="dsl__register" id="register" aria-labelledby="dsl-s10">
            ...
            <div className="dsl__reggrid">
              <div className="dsl__regmain">
                ...
                {/* opens the file itself rather than routing to the
                    general project brief, which asks a different set of
                    questions and knows nothing about courses */}
                <LabRegisterButton className="dsl__regcta">
                  {up(R.beginRegistration)}
                  <LabArrow />
                </LabRegisterButton>
              </div>
              <div className="dsl__regside">
                <span className="dsl__regask">{up(t(LAB_REG_COPY.fileAsks, locale))}</span>
                {/* the same array the file itself renders, so what the page
                    promises and what the form asks cannot drift apart */}
                <ul className="dsl__regfields">
                  {LAB_REG_FIELDS.map((f) => (
                    <li key={f.en}>{t(f, locale)}</li>
                  ))}
                </ul>
                <span className="dsl__regnote">{up(t(LAB_REG_COPY.enrolmentNote, locale))}</span>
              </div>
            </div>
          </section>
